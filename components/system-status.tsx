...
export function SystemStatus({ 
  ...
  const getStatusColor = (status: string): string => {
    if (status === 'available' || status === 'supported' || status === 'loaded') return "bg-green-500";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="flex items-center">
            ...
            <div>
              ...
              <span>{webGPUStatus === 'available' ? 'Available' : 'Not Available'}</span>
            </div>
          </div>
          <div className="flex items-center">
            <span 
            ...
            <div>
              ...
              <span>{fp16Status === 'supported' ? 'Supported' : 'Not Supported'}</span>
            </div>
          </div>
          <div className="flex items-center">
            <span 
          ...
          <div className="mt-4">
            ...
            {progress.overall && (
              <div className="flex justify-between text-sm text-muted-foreground mt-2">
                <div>
                  <span>{progress.overall.formattedLoaded || '0 B'}</span> / <span>{progress.overall.formattedTotal || '0 B'}</span>
                </div>
                <div>
                  <span>{progress.overall.formattedSpeed || '0 B/s'}</span> • <span>{progress.overall.formattedRemaining || '--'}</span>
                </div>
              </div>
            )}
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <div className="text-xs text-muted-foreground">
                        ...
                        {file.bytesLoaded !== undefined && file.bytesTotal !== undefined && (
                          <span className="mr-2">{formatBytes(file.bytesLoaded)} / {formatBytes(file.bytesTotal)}</span>
                        )}
                        {file.speed && <span className="mr-2">{formatBytes(file.speed)}/s</span>}
                        {file.timeRemaining && <span>ETA: {formatTime(file.timeRemaining)}</span>}
                      </div>
                    </div>
                    <Progress value={file.percentComplete} className="h-1 mt-1" />
                  </div>
                ))}
              </div>
            )}
